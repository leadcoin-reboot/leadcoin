...
uint256 hashGenesisBlock("0x505d9bf0e6ac3cf09e6e13e0925cd19f4191846c5f7d62c7518ed0079edd36b4");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Leadcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Leadcoin Signed Message:\n";
...
{
    int64 nSubsidy = 3840 * COIN;

    // Subsidy is cut in half every 86400 blocks which will occur approximately every 1 months.
...
static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Leadcoin old retarget: 1 days
static const int64 nTargetTimespanNew = 30; // Leadcoin: 0.5 minutes
static const int64 nTargetSpacing = 30; // Leadcoin: 0.5 minutes
static const int64 nInterval = nTargetTimespan / nTargetSpacing;

static const int64 nDiffChangeTarget = 308000; // Switch to DigiShield at block 308000
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Leadcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Leadcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Leadcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x505d9bf0e6ac3cf09e6e13e0925cd19f4191846c5f7d62c7518ed0079edd36b4");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Independence Day is one of America's principal legal holiday.July 4,1776";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("042d40113d94d2f5eb49c7f0f453c6c3ea9bc02898f3d095577df2aaab08e6b2bb939ebe9afd5e516f7ec717d3e62ebe386f0fa1aa7ea76ae39242793b28a63f12") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1387900800;
        ...
        assert(block.hashMerkleRoot == uint256("0x7d40a20b704fbab4dd2cb34810f724c5f3573ed8b9278535ad4f6a31784ff698"));
	    	block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Leadcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LeadcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LeadcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LeadcoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running LeadcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LeadcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LeadcoinMiner, pwallet));
}
